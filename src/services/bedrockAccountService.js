    ...
    try {
      ...
      if (updates.schedulable !== undefined) {
        ...
        if (!isAutoDisable) {
          ...
          account.rateLimitAutoStopped = ''
          account.quotaAutoStopped = ''
          // 兼容旧的标记
          account.autoStoppedAt = ''
          account.stoppedReason = ''

          // 记录手动修改日志
          if (updates.schedulable === true || updates.schedulable === 'true') {
            logger.info(`✅ Manually enabled scheduling for Bedrock account ${accountId}`)
          } else {
            logger.info(`⛔ Manually disabled scheduling for Bedrock account ${accountId}`)
          }
        }
